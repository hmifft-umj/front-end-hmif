...
import useCreateDataArtikel from "../_hooks/useCreateDataArtikel";
...
const CardCreateDataArtikel = () => {
  useCsrfToken();
  const { form, isLoading, onSubmit } = useCreateDataArtikel();
    ...
    <Form {...form}>
      ...
      >
        <Card className="border-2 border-primary">
          <CardHeader>
            <CardTitle>Create Data Artikel</CardTitle>
            <CardDescription>Form for create data artikel</CardDescription>
          </CardHeader>
          <CardContent>
            <FormInput
              form={form.control}
            ...
            />
            <FormFile form={form.control} name={"thumbnail"} />
            {fields.map((content, id) => {
              return (
                <div
                  className="flex w-full flex-col gap-4 pt-4"
                  key={content.id}
                >
                  <TypographyH3>Artikel Content #{id + 1}</TypographyH3>
                  <FormField
                    control={form.control}
                    name={`artikelContent.${id}.tipe`}
                    render={({ field }) => (
                  ...
                  {content.tipe === ArtikelContentTipeEnum.image ? (
                    <FormFile
                      form={form.control}
                      name={`artikelContent.${id}.content`}
                    />
                  ...
                  {content.tipe === "code" ? (
                    <FormSelect
                      form={form.control}
                      name={`artikelContent.${id}.subTipe`}
                      values={[
                        "default",
                        "javascript",
                        "typescript",
...
export default CardCreateDataArtikel;
